...
// ... existing code ...
 if (window.updateDashboard) {
     const existingUpdateDashboard = window.updateDashboard;
     window.updateDashboard = function() {
         existingUpdateDashboard();
         updateQuickStats();
...
// Remove the duplicate declaration and just update the function if it exists
if (window.updateDashboard) {
    const existingUpdateDashboard = window.updateDashboard;
    window.updateDashboard = function() {
        existingUpdateDashboard();
        ...
        <div class="payslip-content">
            <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto; padding: 20px;">
                ...
                <div style="text-align: center; margin: 20px 0; padding: 10px; background: #f8f9fa;">
                    ...
                    <p>Payment Date: ${employee.date}</p>
...
// Add these functions at the top of work.js

// Quick Stats Functions
function updateQuickStats() {
    // Calculate today's payments
    const today = new Date().toDateString();
    const todayPayments = students.reduce((sum, student) => {
        const paymentDate = new Date(student.date).toDateString();
        return paymentDate === today ? sum + student.feesPaid : sum;
    }, 0);
    ...
    const weeklyTotal = students.reduce((sum, student) => {
        const paymentDate = new Date(student.date);
        return paymentDate >= weekStart ? sum + student.feesPaid : sum;
    }, 0);
    document.getElementById('weeklyTotal').textContent = weeklyTotal.toFixed(2);

    // Calculate monthly progress
    const monthlyTarget = 100000; // Example target
    const currentMonth = new Date().getMonth();
    const monthlyTotal = students.reduce((sum, student) => {
        const paymentDate = new Date(student.date);
        return paymentDate.getMonth() === currentMonth ? sum + student.feesPaid : sum;
    }, 0);
    const progress = (monthlyTotal / monthlyTarget) * 100;
...
function initializePaymentTrendsChart() {
    ...
    const monthlyData = getMonthlyPaymentData();
...
function getMonthlyPaymentData() {
    ...
    const monthlyPayments = students.reduce((acc, student) => {
        const date = new Date(student.date);
        const monthKey = `${date.getFullYear()}-${date.getMonth()}`;
        acc[monthKey] = (acc[monthKey] || 0) + student.feesPaid;
        return acc;
    }, {});
    ...
    for (let i = 5; i >= 0; i--) {
        ...
        values.push(monthlyPayments[monthKey] || 0);
    }
...
function generateReport() {
    const reportData = {
        ...
        totalFees: students.reduce((sum, student) => sum + student.feesPaid, 0),
    ...
    const doc = new jsPDF();
    doc.text('School Payment Report', 20, 20);
    doc.text(`Total Students: ${reportData.totalStudents}`, 20, 40);
    doc.text(`Total Fees Collected: K${reportData.totalFees.toFixed(2)}`, 20, 50);
...
function exportData() {
    ...
    link.download = 'student-payments.csv';
    link.click();
}
...
function updateBulkFees() {
    const grade = document.getElementById('bulkGradeSelect').value;
    const newFee = parseFloat(document.getElementById('bulkFeeAmount').value);
    
    if (!grade || isNaN(newFee)) {
        alert('Please select a grade and enter a valid fee amount');
        return;
    }
...
function sendBatchNotifications() {
    ...
    switch(template) {
        case 'payment':
            message = 'Please note that school fees are due. Kindly make payment as soon as possible.';
            break;
        case 'update':
            message = 'School fees have been updated. Please check the new fee structure.';
            break;
        case 'custom':
    ...
    showNotification(`Notifications sent to ${sentCount} recipients`);
}

// Helper Functions
function convertToCSV(data) {
    const headers = ['Student Number', 'Name', 'Grade', 'Fees Paid', 'Balance', 'Date'];
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listener for notification template
    const templateSelect = document.getElementById('notificationTemplate');
    const customMessage = document.getElementById('customMessage');
    if (templateSelect && customMessage) {
        templateSelect.addEventListener('change', function() {
            customMessage.style.display = this.value === 'custom' ? 'block' : 'none';
        });
    }

    // Add close button functionality for bulk operations modal
    const bulkOperationsModal = document.getElementById('bulkOperationsModal');
    const closeBtn = bulkOperationsModal.querySelector('.close');
    if (closeBtn) {
        closeBtn.onclick = function() {
            bulkOperationsModal.style.display = 'none';
        };
    }
});

// Single implementation of dashboard update
// ... existing code ...

// Remove any previous declarations and use a single implementation
window.updateDashboard = (function() {
...
// ... existing code ...
window.updateDashboard = updateDashboardWithFeatures;

// Add these helper functions
function getGradeDistribution() {
    const distribution = {};
    for (let i = 1; i <= 9; i++) {
        distribution[i] = students.filter(s => s.grade === i).length;
    }
    return distribution;
}
...
function getPaymentStatus() {
    return {
        paid: students.filter(s => s.balance <= 0).length,
        partial: students.filter(s => s.balance > 0 && s.feesPaid > 0).length,
        unpaid: students.filter(s => s.feesPaid === 0).length
    };
}

// Update the initialization to ensure it runs after DOM and data are loaded
document.addEventListener('DOMContentLoaded', function() {
    if (!students || !students.length) {
        setTimeout(initializeFeatures, 1000); // Retry if data not loaded
        return;
    }
    ...
    // Initialize event listeners for bulk operations
    const bulkOperationsModal = document.getElementById('bulkOperationsModal');
